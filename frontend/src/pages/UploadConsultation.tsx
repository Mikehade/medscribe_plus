...
function FileIcon() {
  return (
    <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
      <rect width="36" height="36" rx="8" fill="var(--ms-surface2)" />
      <path d="M10 9h10l6 6v12a2 2 0 01-2 2H10a2 2 0 01-2-2V11a2 2 0 012-2z" stroke="var(--ms-accent)" strokeWidth="1.5" fill="none" />
      <path d="M20 9v6h6" stroke="var(--ms-accent)" strokeWidth="1.5" fill="none" />
      <path d="M14 20h8M14 24h5" stroke="var(--ms-muted)" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}
...
function UploadIcon() {
  return (
    <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
      <circle cx="20" cy="20" r="19" stroke="var(--ms-border)" strokeWidth="1.5" strokeDasharray="3 2.5" />
      <path d="M20 28V18M15 23l5-5 5 5" stroke="var(--ms-accent)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
export default function UploadConsultation() {
  ...
  const pickFile = (f: File) => {
    ...
    setError(null); setFile(f); setResult(null); setApproveStatus("idle");
  };
  ...
  const upload = async () => {
    if (!file) return;
    setError(null); setResult(null); setUploadState("uploading"); simulateProgress();
    const form = new FormData();
    form.append("file", file); form.append("patient_id", patientId);
    try {
      ...
      clearInterval(progressRef.current!); setProgress(100); setResult(data); setUploadState("done");
    } catch (err: any) {
      clearInterval(progressRef.current!); setError(err.message || "Upload failed"); setUploadState("error");
    }
  };

  const approve = async () => {
    if (!result) return; setApproveStatus("loading");
    try {
  ...
  const reset = () => {
    setUploadState("idle"); setFile(null); setResult(null); setError(null);
    setProgress(0); setApproveStatus("idle"); setEhrId(null);
  };
    ...
    <div className="ms-page">
      ...
      <div className="ms-page-header">
        <div className="ms-header-left">
          ...
          <div className="ms-page-title">Upload Consultation</div>
        </div>
        <div className="ms-page-sub">Process a pre-recorded consultation audio file</div>
      </div>
      ...
      <div className="ms-layout">
        ...
        <div className="ms-left">
          <div className="ms-panel">
            <div className="ms-section-label">Audio File</div>
            ...
            {!file ? (
              ...
              >
                <UploadIcon />
                <p className="ms-drop-title">Drop audio file here</p>
                <p className="ms-drop-sub">or click to browse<br />WAV · MP3 · WebM · OGG · M4A · max 100 MB</p>
                <input ref={fileInputRef} type="file" accept={ACCEPTED_EXT} style={{ display: "none" }}
              ...
              <div className="ms-file-preview">
                ...
                {!isLoading && <button className="ms-btn-icon-only" onClick={reset} title="Remove">✕</button>}
              </div>
            )}

            <div className="ms-field">
              <label className="ms-field-label">Patient ID</label>
              <input className="ms-input" value={patientId} onChange={(e) => setPatientId(e.target.value)}
                placeholder="e.g. P001" disabled={isLoading} />
            </div>
            ...
            <div className="ms-recorder-actions">
              {!isLoading && uploadState !== "done" && (
                <button className="ms-btn ms-btn-primary" onClick={upload} disabled={!file || !!error}>
                  Process Consultation
                </button>
              )}
              {uploadState === "done" && (
                <button className="ms-btn ms-btn-ghost" onClick={reset}>Upload Another</button>
              )}
            </div>
          </div>
          ...
          {result && (
            <div className="ms-panel" style={{ flex: 1 }}>
              <div className="ms-card-header">
                <span className="ms-section-label" style={{ marginBottom: 0 }}>Transcript</span>
        ...
        <div className="ms-right">
          {isLoading && (
            <div className="ms-processing-card">
              ...
              <div className="ms-processing-steps">
                {["Transcribing audio", "Generating SOAP note", "Checking drug interactions", "Scoring compliance"].map((step, i) => (
              ...
              <div className="ms-results-wrapper" style={{ flex: 1, borderRadius: 0, border: "none", borderTop: "1px solid var(--ms-border)" }}>
                ...
                <div className="ms-tab-content" style={{ flex: 1, overflowY: "auto" }}>
                  ...
                  {activeTab === "transcript" && (
                    <div className="ms-transcript-full">{result.transcript || "No transcript available."}</div>
                  )}
                </div>
              </div>

              {/* Approve bar */}
              <div style={{ padding: "16px 20px", borderTop: "1px solid var(--ms-border)", background: "var(--ms-surface)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
                ...
                <div className="ms-approve-right">
                  {approveStatus === "idle"    && <button className="ms-btn ms-btn-approve" style={{ width: "auto" }} onClick={approve}>Approve &amp; Insert →</button>}
                  {approveStatus === "loading" && <span className="ms-status-label amber">Inserting…</span>}
                  {approveStatus === "done"    && <span className="ms-status-label green">✓ Inserted {ehrId && `#${ehrId}`}</span>}
                  {approveStatus === "error"   && <button className="ms-btn ms-btn-danger" style={{ width: "auto" }} onClick={approve}>Retry</button>}
                </div>
              </div>
            </div>
          )}
          ...
          {!result && !isLoading && (
            <div className="ms-empty-state">
              ...
              <p className="ms-empty-text">Upload a consultation recording<br />to generate SOAP notes &amp; scores</p>
            </div>
          )}
        </div>
      </div>
